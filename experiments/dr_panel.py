...
if __name__ == "__main__":
    ...
    dataset_file = os.path.join(input_dir, 'dataset_panel.pkl')
    true_atts_file = os.path.join(input_dir, 'true_atts_panel.pkl')
    ...
    if os.path.exists(dataset_file) and os.path.exists(true_atts_file):
        ...
        cnames = [col for col in dataset[0].columns if col not in ['Y_1', 'Y_0', 'G', 'D']]
        ...
        futures = Parallel(n_jobs=max_workers, )(
            delayed(generate_synthetic_data)(setting=setting, num_samples=max(sample_sizes), num_features=num_features,
                                             random_seed=seed)
    ...
    for num_samples in sample_sizes:
        ...
        for models in ['correct', 'or_misspec', 'ps_misspec', 'both_misspec']:
            ...
            results_file = os.path.join(output_dir, 'v13_' + str(int(sample_sizes[-1]/1000)) + 'k.pkl')
